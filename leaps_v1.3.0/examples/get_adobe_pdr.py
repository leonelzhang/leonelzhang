...
def get_adobe_5year_data():
    ...
    print(f"\n正在从 {data_source} 获取 {symbol} 近5年的历史数据...")
    ...
    try:
        ...
        hist = web.DataReader(symbol, data_source, start_date, end_date)
        
        hist = hist.sort_index()
        
        print(f"✓ 成功获取 {len(hist)} 条数据")
        print(f"实际数据时间范围: {hist.index[0].strftime('%Y-%m-%d')} 至 {hist.index[-1].strftime('%Y-%m-%d')}")
